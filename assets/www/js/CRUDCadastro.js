...
function Listar_usuarios(){
    var Parent = document.getElementById('corpo');
    while(Parent.hasChildNodes())
    {
       Parent.removeChild(Parent.firstChild);
    }

    Parent.innerHTML = '<th>ID</th><th>NOME</th><th>BAIRRO</th><th>CIDADE</th><th>UF</th><th>PESSOAS</th><th>MENORES</th><th>DISTÂNCIA</th>';
    
	query = "SELECT * FROM Cidadao";
    try {
        localDB.transaction(function(transaction){
            transaction.executeSql(query, [], function(transaction, results){
                for (var i = 0; i < results.rows.length; i++) {
                    ...
                    tr.setAttribute("style", "background-color:ligthgray;color:black");
                    ...
                    td.appendChild(document.createTextNode(row['nome']));
                    tr.appendChild(td);

                    td = document.createElement('td');
                    td.appendChild(document.createTextNode(row['bairro']));
                    tr.appendChild(td);

                    td = document.createElement('td');
                    td.appendChild(document.createTextNode(row['cidade']));
                    tr.appendChild(td);

                    td = document.createElement('td');
                    td.appendChild(document.createTextNode(row['uf']));
                    tr.appendChild(td);

                    td = document.createElement('td');
                    td.appendChild(document.createTextNode(row['qtpessoas']));
                    tr.appendChild(td);

                    td = document.createElement('td');
                    td.appendChild(document.createTextNode(row['qtmenores']));
                    tr.appendChild(td);

                    td = document.createElement('td');
                    td.appendChild(document.createTextNode(row['distancia']));
                    tr.appendChild(td);
